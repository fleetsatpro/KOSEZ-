            ...
            {calendar.map((item) => (
              ...
              >
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 text-xs text-subtle">Événement</p>
                </div>
                <span className="shrink-0 text-xs tabular-nums text-muted">{item.when}</span>
              </li>
            ))}
          </ul>
        )}
      ...
      <Surface className="mt-4 !p-5 sm:!p-6">
        ...
        <ul className="mt-4 space-y-2">
          {proof.lines.map((line) => (
            <li key={line} className="flex items-center gap-2 text-sm tabular-nums text-fg">
              ...
              <li key={lang.id}>
                ...
                >
                  <span>
                    ...
                    <span className="mt-0.5 block text-xs text-subtle">{lang.blurb}</span>
                  </span>
                  {active ? <Badge>Actif</Badge> : <ChevronRight className="size-4 text-subtle" />}
                </button>
              </li>
            );
          })}
