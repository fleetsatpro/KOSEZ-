...
function GrammarLab() {
  ...
  function next() {
    if (!answered) return;
    if (index >= GRAMMAR_TASKS.length - 1) { saveLearningSubmission({ taskId: task.id, kind: "grammar", content: choice ?? "", checks: [choice === task.answer ? "correct" : "incorrect"], result: { correct: choice === task.answer, target: task.target } }); completeActivity("GRAMMAR_COMPLETED", dailyLabSource("grammar", task.id), `Grammaire · ${correct + (choice === task.answer ? 1 : 0)}/${GRAMMAR_TASKS.length}`); setFinished(true); return; }
