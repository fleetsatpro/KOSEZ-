...
-- Learner evidence submissions. Content is durable, owner-scoped and
-- idempotent through the mutation id used as the submission id.
create table if not exists blossom_learning_submission (
  id uuid primary key,
  user_id text not null,
  task_id text not null,
  kind text not null check (kind in ('grammar','listening','writing')),
...
create index if not exists blossom_submission_user_time_idx
  on blossom_learning_submission (user_id, created_at desc);

create unique index if not exists blossom_submission_user_task_id_idx
  on blossom_learning_submission (user_id, task_id, id);
